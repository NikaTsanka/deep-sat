...
def run_simple_net():
   ...
   # conv4_pool = max_pool_2x2(conv4) # 7x7x64 -> 7x7x96
    ...
    def test(sess):
        ...
        acc = np.mean([sess.run(accuracy, feed_dict={x: X[i], y_: Y[i], keep_prob: 1.0, phase_train: False})
                       for i in range(10)])
        return acc
    ...
    with tf.Session() as sess:
        ...
        for i in range(STEPS):
            ...
            if i % 500 == 0:
                acc = test(sess)
                loss = sess.run(predict, feed_dict={x: batch_x, y_: batch_y, keep_prob: dropoutProb, phase_train: False})
